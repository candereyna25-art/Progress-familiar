...
import {healthScore,bmi,bmiClass,waistHeight} from './health.js';
...
async function ui(){$('auth').classList.toggle('hidden',!!session);$('app').classList.toggle('hidden',!session);$('nav').classList.toggle('hidden',!session);if(session)await load()}
async function load(){let a=await sb.from('profiles').select('*').eq('id',session.user.id).single();p=a.data;let b=await sb.from('progress_entries').select('*').eq('user_id',session.user.id).order('entry_date');rows=b.data||[];$('name').value=p?.full_name||'';$('height').value=p?.height_cm||'';$('mid').value=p?.intermediate_goal_kg||'';$('final').value=p?.final_goal_kg||'';$('hello').textContent='Hola, '+(p?.full_name||'');let f=await sb.from('families').select('*');$('familyCode').textContent=(f.data||[]).find(x=>x.owner_id===session.user.id)?.invite_code||'MIEMBRO';render()}
function render(){if(!rows.length){$('list').innerHTML='Sin registros';drawChart($('chart'),rows,chart);return}
 const first=rows[0],last=rows.at(-1),prev=rows.at(-2),goal=Number(p?.intermediate_goal_kg||p?.final_goal_kg||0),score=healthScore(last);
 $('weightNow').textContent=Number(last.weight_kg).toFixed(2)+' kg';$('score').textContent=score;$('scoreRing').style.background='conic-gradient(var(--pri) '+score*3.6+'deg,#e8eeee 0deg)';
 const wd=prev?Number(last.weight_kg)-Number(prev.weight_kg):0;$('weeklyDelta').textContent=prev?(wd<=0?'↓ ':'↑ ')+Math.abs(wd).toFixed(2)+' kg vs anterior':'Primer registro';
 const b=bmi(Number(last.weight_kg),Number(p?.height_cm||0));$('bmi').textContent=b?b.toFixed(1):'—';$('bmiClass').textContent=b?bmiClass(b):'Configura estatura';
 const waist=[...rows].reverse().find(r=>r.waist_cm!=null),ratio=waistHeight(Number(waist?.waist_cm||0),Number(p?.height_cm||0));$('whr').textContent=ratio?ratio.toFixed(2):'—';$('whrText').textContent=waist?'Cintura '+waist.waist_cm+' cm':'Registra cintura';
 renderPeriod(7,'change7','change7Text');renderPeriod(30,'change30','change30Text');
 $('goalTitle').textContent=goal?'Camino a '+goal+' kg':'Camino a tu meta';const pc=goal?Math.max(0,Math.min(100,(first.weight_kg-last.weight_kg)/(first.weight_kg-goal)*100)):0;$('progressBar').style.width=pc+'%';$('goalText').textContent=goal?'Avance '+pc.toFixed(0)+'% · faltan '+Math.max(0,last.weight_kg-goal).toFixed(2)+' kg':'Configura meta';
 const recent=rows.slice(-4);if(goal&&recent.length>=2){const days=(new Date(recent.at(-1).entry_date)-new Date(recent[0].entry_date))/86400000,rate=(recent[0].weight_kg-recent.at(-1).weight_kg)/(days||1);$('projection').textContent=rate>0?Math.ceil((last.weight_kg-goal)/rate)+' días aprox.':'Sin proyección'}
 renderMilestones(last.weight_kg);renderCalculated(first,last);renderEngineering(last,prev);$('coach').textContent=coachText(last);renderList();drawChart($('chart'),rows,chart);
}
function renderPeriod(days,id,textId){const old=nearestBefore(days),last=rows.at(-1);if(!old){$(id).textContent='—';$(textId).textContent='Sin datos suficientes';return}const d=Number(last.weight_kg)-Number(old.weight_kg);$(id).textContent=(d>0?'+':'')+d.toFixed(2)+' kg';$(textId).textContent=d<0?'Bajaste en el periodo':d>0?'Subiste en el periodo':'Sin cambio'}
function renderMilestones(weight){$('milestones').innerHTML=[125,120,115,110,105,100,95].map(x=>'<span class="milestone '+(weight<=x?'done':'')+'">'+x+' kg</span>').join('')}
